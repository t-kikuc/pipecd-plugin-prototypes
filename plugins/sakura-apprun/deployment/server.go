...
type DeploymentServiceServer struct {
	...
	deployTargetConfig apprunconfig.AppRunDeployTargetConfig

	logger       *zap.Logger
	logPersister logPersister
}

// NewDeploymentServiceServer creates a new DeploymentServiceServer of this plugin.
func NewDeploymentServiceServer(
	...
	if len(config.DeployTargets) > 0 {
		var err error
		if deployTargetConfig, err = apprunconfig.ParseDeployTargetConfig(config.DeployTargets[0]); err != nil {
			return nil, err
		}
	}
	...
	return &DeploymentServiceServer{
		...
		logger:             logger.Named("apprun-plugin"),
		logPersister:       logPersister,
	}, nil
}
...
func (s *DeploymentServiceServer) DetermineStrategy(ctx context.Context, request *deployment.DetermineStrategyRequest) (*deployment.DetermineStrategyResponse, error) {
	cfg, err := config.DecodeYAML[*apprunconfig.AppRunApplicationSpec](request.GetInput().GetTargetDeploymentSource().GetApplicationConfig())
	if err != nil {
